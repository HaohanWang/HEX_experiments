...
def oneHotRepresentation(y, k=10):
    n = y.shape[0]
    r = np.zeros([n, k])
...
def addMultiDomainPattern(r, randomMask=None, radioMask=None):
    p = np.random.random()
    if p < 1.0/3:
        return r, 0
    elif p < 2.0/3:
        return addingPattern(r, randomMask), 1
    else:
        return addingPattern(r, radioMask), 2
...
def loadMultiDomainMNISTData(testCase=1):
    ...
    trainingPatternLabel = []
    validationPatternLabel = []
    testingPatternLabel = []

    for i in range(training_data[0].shape[0]):
        r = training_data[0][i]
        r=r.reshape(28,28)
        img, l = addMultiDomainPattern(r, randomMask=randomMask, radioMask=radioMask)
        _Xtrain[i]=img.reshape(1,28*28)
        trainingPatternLabel.append(l)
    ...
    for i in range(validation_data[0].shape[0]):
        ...
        validationPatternLabel.append(l)
    ...
    for i in range(test_data[0].shape[0]):
        ...
        testingPatternLabel.append(l)

    trainingPatternLabel = np.array(trainingPatternLabel)
    validationPatternLabel = np.array(validationPatternLabel)
    testingPatternLabel = np.array(testingPatternLabel)

    indices = np.random.permutation(_Xtrain.shape[0])
    _Xtrain = _Xtrain[indices, :]
    training_label = training_data[1][indices]
    trainingPatternLabel = trainingPatternLabel[indices]

    np.save('trainingPatternLabel', trainingPatternLabel)

    return _Xtrain, oneHotRepresentation(training_label),_Xvalidation,oneHotRepresentation(validation_data[1]),_Xtest,oneHotRepresentation(test_data[1]), \
            oneHotRepresentation(trainingPatternLabel, 3), oneHotRepresentation(validationPatternLabel, 3), oneHotRepresentation(testingPatternLabel, 3)
