...
from cnn_v2_baseline import MNISTcnn
...
def load_and_deal(corr,row,column,ngray=16, testCase=0):
    ...
    xtrain, ytrain, xval, yval, xtest, ytest = loadMultiDomainMNISTData(testCase=testCase)
    ...
    xtrain_re=xtrain_re.reshape(xtrain_re.shape[0],1,784)
    #print (xtrain[0].shape)

    # xva=np.repeat(xval_re,ngray,0)
    xval_re=xval_re.reshape(xval_re.shape[0],1,784)

    # xte=np.repeat(xtest_re,ngray,0)
    xtest_re=xtest_re.reshape(xtest_re.shape[0],1,784)

    ################# delta ################
    # xtr_d=np.repeat(xtrain_d,ngray,0)
    xtrain_d=xtrain_d.reshape(xtrain_d.shape[0],1,784)
    #print (xtr_d.shape)
    #print(xtrain_d.shape)
    
    # xva_d=np.repeat(xval_d,ngray,0)
    xval_d=xval_d.reshape(xval_d.shape[0],1,784)
    #print (xva_d.shape)
    #print (xval_d.shape)
    
    # xte_d=np.repeat(xtest_d,ngray,0)
    xtest_d=xtest_d.reshape(xtest_d.shape[0],1,784)
    ################# delta ################
