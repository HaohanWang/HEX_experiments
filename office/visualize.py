...
def loadTxt(filename):
    ...
    for i in range(5):
        updateTest = True
        maxVal = 0
        text = [line.strip() for line in open('../results/office/'+ filename + '_' + str(i) + '.txt')]
        tr = []
        val = []
        te = []
        for line in text:
...
def plot(ds):
    tr1, val1, te1 = loadTxt('baseline_'+str(ds))
    tr2, val2, te2 = loadTxt('hex_'+str(ds))
    ...
    plt.legend(loc=4)
    plt.savefig('office_'+str(ds)+'.pdf')
    plt.clf()
